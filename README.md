<!-- 标题区 -->
<h1 align="center">Hi 👋，这里是一个正在进化的程序员</h1>
<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&pause=1000&center=true&vCenter=true&multiline=true&width=435&lines=爱写代码的搬砖工;Bug是我最亲密的朋友;白天写Bug, 晚上梦Bug" alt="Typing SVG" />
</p>

---

## 🧙 关于我

- 🌱 正在偷偷学习如何写出不会报错的代码
- 🧰 技能树歪歪斜斜但有点绿
- 🐛 bug 重灾户，已经和调试工具成为灵魂伴侣
- 💬 喜欢用代码说话，但说着说着常常语无伦次
- 🧪 技术关键词：`Spring Boot`、`Vue`、`Python`、`Flask`、`Django`、`嵌入式`、`爬虫`

---

## 🧰 我的技术栈（不全，但用心）

<p align="left">
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white" />
  <img src="https://img.shields.io/badge/Vue.js-42b883?style=for-the-badge&logo=vue.js&logoColor=white" />
  <img src="https://img.shields.io/badge/Spring_Boot-6DB33F?style=for-the-badge&logo=spring-boot&logoColor=white" />
  <img src="https://img.shields.io/badge/Flask-black?style=for-the-badge&logo=flask&logoColor=white" />
  <img src="https://img.shields.io/badge/Django-092E20?style=for-the-badge&logo=django&logoColor=white" />
  <img src="https://img.shields.io/badge/Embedded-Critical?style=for-the-badge&logo=c&logoColor=white" />
</p>

---

## 🏆 GitHub 成就墙

<p align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=你的GitHub用户名&theme=monokai&row=1&column=6" />
</p>

---

## 📈 我的 GitHub 数据（都是真实摸鱼的痕迹）

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=你的GitHub用户名&show_icons=true&theme=tokyonight" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=你的GitHub用户名&layout=compact&theme=tokyonight" />
</p>

---

## 🐾 最近在忙啥？

<!-- wakatime 报告 -->
<!-- 需要在 wakatime.com 注册账号并获取 API Key -->
<!-- <img src="https://github-readme-stats.vercel.app/api/wakatime?username=你的用户名" /> -->

其实最近忙着...调一个昨天还能跑今天跑不了的项目。

---

## 📫 联系方式（除了鸽子啥都能回）

- GitHub: [@你的GitHub用户名](https://github.com/你的GitHub用户名)
- Email: your_email@example.com

---

## 🐣 彩蛋时刻

<p align="center">
  <img src="https://raw.githubusercontent.com/你的GitHub用户名/你的GitHub用户名/output/github-contribution-grid-snake.svg" alt="snake" />
</p>

---

## 👀 谁来看过我？

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=你的GitHub用户名&style=flat-square&color=blue" alt="访客统计">
</p>

---

⭐ 如果你觉得我写得还行，不妨点个 Star，让我知道还有人在看我胡言乱语 😂
